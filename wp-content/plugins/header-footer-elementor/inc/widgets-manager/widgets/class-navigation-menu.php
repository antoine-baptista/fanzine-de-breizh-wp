	...
	protected function render() {
		...
		if ( 'flyout' === $settings['layout'] ) {
					...
					<div id="hfe-flyout-content-id-<?php echo esc_attr( $this->get_id() ); ?>" class="hfe-side hfe-flyout-<?php echo esc_attr( $settings['flyout_layout'] ); ?> hfe-flyout-open" data-layout="<?php echo wp_kses_post( $settings['flyout_layout'] ); ?>" data-flyout-type="<?php echo wp_kses_post( $settings['flyout_type'] ); ?>">
						<div class="hfe-flyout-content push">
							...
							<?php
									...
									// s'il est connecté, nous afficherons le menu contributeur
									// retourne un booléen vrai si l'utilisateur est connecté
									// faux dans le cas contraire
								if(is_user_logged_in()) {
									// wp_nav_menu affiche un menu de navigation, sans ce cas je veux que
									// ce soit le menu 'menu_contributeur' qui s'affiche à vrai
									$htmlaAfficher = wp_nav_menu(['menu' => 'menu_contributeur']);
								}
								else{
									// sinon nous afficherons le menu par défaut de WordPress
									// on donnera à $htmlaAfficher la valeur du menu par défaut
									// de WordPRess
									$htmlaAfficher = $menu_html;
								}
							?>
									<!-- juste en dessous la première ligne affiche la div avec la classe hfe-nav-menu
									en regardant avec l'inspecteur, c'est bien dans cette div que se trouve les menus
									la ligne suivante affiche le menu qui a été sélectionné par la boucle if
									La fonction wp_kses_post désinfecte le contenu pour les balises HTML autorisées pour
									contenu de la publication -- Cette fonction attend des données non barrées -->
							<nav <?php echo wp_kses_post( $this->get_render_attribute_string( 'hfe-nav-menu' ) ); ?>>
								<?php echo $htmlaAfficher; ?>
							</nav>
							<!-- fin modif code AB -->				
			...
			<div <?php echo $this->get_render_attribute_string( 'hfe-main-menu' ); ?>>
				...
				<nav <?php echo $this->get_render_attribute_string( 'hfe-nav-menu' ); ?>></nav>
			</div>
			<?php
		}
	}
